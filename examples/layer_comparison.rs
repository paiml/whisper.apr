...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  RIFF marker: {:?} (expected: RIFF)",
        String::from_utf8_lossy(riff)
    );
    println!(
        "  WAVE marker: {:?} (expected: WAVE)",
        String::from_utf8_lossy(wave)
    );
    println!(
        "  Format chunk size: {} (expected: 16 for PCM)",
        fmt_chunk_size
    );
    ...
    while data_offset < audio_bytes.len() - 8 {
        ...
        let chunk_size = u32::from_le_bytes([
            audio_bytes[data_offset + 4],
            audio_bytes[data_offset + 5],
            audio_bytes[data_offset + 6],
            audio_bytes[data_offset + 7],
        ]);
        if chunk_id == b"data" {
            data_offset += 8;
            break;
    ...
    println!(
        "  Dynamic range: {:.2} dB",
        20.0 * (max / min.abs()).log10()
    );

    // H0 falsification test
    let h0_pass =
        sample_rate == 16000 && num_channels == 1 && bits_per_sample == 16 && std_dev > 0.01;
    println!(
        "\n✓ H0 (Valid Audio): {} - {}",
        if h0_pass { "PASS" } else { "FAIL" },
        if h0_pass {
            "Audio samples are valid"
        } else {
            "Audio may be corrupt or silent"
        }
    );
    ...
    println!(
        "  N_frames: {} (expected: ~150 for 1.5s @ 100fps)",
        n_frames
    );
    ...
    println!("\nNormalization Check:");
    println!(
        "  In [-2, 2] range: {} (whisper.cpp uses (x+4)/4 → [-1,1])",
        in_normalized_range
    );
    ...
    for frame in 0..n_frames.min(10) {
        ...
        if frame_end <= mel_data.len() {
            let frame_mean: f32 =
                mel_data[frame_start..frame_end].iter().sum::<f32>() / n_mels as f32;
    ...
    println!(
        ...
        if h1_pass {
            "Mel spectrogram has valid shape and variance"
        } else {
            "Mel spectrogram may be malformed"
        }
    );
    ...
    let unique_approx = enc_data
        .iter()
        .map(|&x| (x * 1000.0) as i32)
        .collect::<std::collections::HashSet<_>>()
        .len();
    println!(
        "  Unique values (approx): {} (expected: many thousands)",
        unique_approx
    );
    ...
    println!(
        ...
        if h2_pass {
            "Encoder output has valid statistics"
        } else {
            "Encoder output may be collapsed/invalid"
        }
    );

    // Dump encoder output
    dump_encoder_to_file(
        &enc_data,
        enc_seq_len,
        d_model,
        "/tmp/whisper_apr_encoder.bin",
    )?;
    ...
    // Top 10 tokens
    let mut indexed: Vec<(usize, f32)> = raw_logits
        .iter()
        .enumerate()
        .map(|(i, &v)| (i, v))
        .collect();
    ...
    let token_265_rank = indexed
        .iter()
        .position(|(id, _)| *id == 265)
        .unwrap_or(n_vocab);
    ...
    // H3: Decoder validity
    let h3_pass = token_265_rank > 5
        || raw_logits[440] > raw_logits[265]
        || raw_logits[383] > raw_logits[265];
    println!(
        "\n✓ H3 (Valid Decoder): {} - {}",
        if h3_pass { "PASS" } else { "FAIL" },
        if h3_pass {
            "Decoder produces sensible logits"
        } else {
            "Decoder is stuck on token 265"
        }
    );

    // Dump logits
    std::fs::write(
        "/tmp/whisper_apr_logits.bin",
        raw_logits
            .iter()
