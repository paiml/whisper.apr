...
fn stats(x: &[f32]) -> (f32, f32, f32, f32) {
    ...
    (mean, std, l2, x.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b.abs())))
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  mean: {:.4}, std: {:.4}, L2: {:.4}, max_abs: {:.4}", pe_mean, pe_std, pe_l2, pe_max);
    ...
    println!("  mean: {:.6}, std: {:.4}, L2: {:.4}, max_abs: {:.4}", w1_mean, w1_std, w1_l2, w1_max);
    let (b1_mean, b1_std, b1_l2, b1_max) = stats(conv1_bias);
    println!("Conv1 bias: mean: {:.6}, std: {:.4}, L2: {:.4}, max_abs: {:.4}", b1_mean, b1_std, b1_l2, b1_max);

    println!("\nConv2 weights ({} values):", conv2_weight.len());
    let (w2_mean, w2_std, w2_l2, w2_max) = stats(conv2_weight);
    println!("  mean: {:.6}, std: {:.4}, L2: {:.4}, max_abs: {:.4}", w2_mean, w2_std, w2_l2, w2_max);
    let (b2_mean, b2_std, b2_l2, b2_max) = stats(conv2_bias);
    println!("Conv2 bias: mean: {:.6}, std: {:.4}, L2: {:.4}, max_abs: {:.4}", b2_mean, b2_std, b2_l2, b2_max);
    ...
    for (i, block) in encoder.blocks().iter().enumerate() {
        ...
        println!("  Block {}: ln1.w mean={:.4} max={:.4}, ln1.b mean={:.4} max={:.4}",
                 i, ln1w_mean, ln1w_max, ln1b_mean, ln1b_max);
        println!("           ln2.w mean={:.4} max={:.4}, ln2.b mean={:.4} max={:.4}",
    ...
    println!("\nEncoder ln_post:");
    println!("  weight: mean={:.4}, std={:.4}, L2={:.4}, max={:.4}", lnpw_mean, lnpw_std, lnpw_l2, lnpw_max);
    println!("  bias:   mean={:.4}, std={:.4}, L2={:.4}, max={:.4}", lnpb_mean, lnpb_std, lnpb_l2, lnpb_max);
    ...
    println!("  mean={:.6}, std={:.4}, L2={:.4}, max={:.4}", mel_mean, mel_std, mel_l2, mel_max);
    ...
    println!("HF:  {:?}", &hf_encoder[0..10].iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
    println!("Our: {:?}", &our_encoder[0..10].iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
    ...
    for pos in 0..5.min(hf_positions) {
        let start = pos * 384;
        let l2: f32 = hf_encoder[start..start+384].iter().map(|x| x*x).sum::<f32>().sqrt();
    ...
    for pos in 0..5.min(our_positions) {
        let start = pos * 384;
        let l2: f32 = our_encoder[start..start+384].iter().map(|x| x*x).sum::<f32>().sqrt();
        println!("  pos {}: L2 = {:.4}", pos, l2);
    }
