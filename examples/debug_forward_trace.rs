...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, &token) in initial_tokens.iter().enumerate() {
        let logits = model.decoder_mut().forward_one(token, &encoded, &mut cache)?;
        let (l_mean, l_std, l_min, l_max) = stats(&logits);
        println!("  Token {}: logits mean={:+.4}  std={:.4}  range=[{:.2}, {:.2}]",
    ...
    let tok_emb: Vec<f32> = token_embed[(token as usize) * d_model..((token as usize) + 1) * d_model].to_vec();
    let (t_mean, t_std, _, _) = stats(&tok_emb);
    println!("  1. Token embed:       mean={:+.6}  std={:.6}", t_mean, t_std);
    ...
    println!("  4. Self-attn Q:       mean={:+.6}  std={:.6}", qs_mean, qs_std);
    ...
    println!("  5. Self-attn K:       mean={:+.6}  std={:.6}", ks_mean, ks_std);
    ...
    println!("  6. Self-attn V:       mean={:+.6}  std={:.6}", vs_mean, vs_std);
    ...
    println!("  7. Self-attn + res:   mean={:+.6}  std={:.6}", r1_mean, r1_std);

    // Step 8: LayerNorm2
    let normed2 = block0.ln2.forward(&residual1)?;
    let (n2_mean, n2_std, _, _) = stats(&normed2);
    println!("  8. LayerNorm2:        mean={:+.6}  std={:.6}", n2_mean, n2_std);
    ...
    println!("  9. Cross-attn Q:      mean={:+.6}  std={:.6}  <-- KEY", qc_mean, qc_std);
    ...
    println!(" 10. Cross-attn K:      mean={:+.6}  std={:.6}", kc_mean, kc_std);

    // Compare: is cross-attn Q healthy now?
    println!("\n=== COMPARISON ===");
    println!("  debug_cross_attn_live Q std: 0.0115 (using raw token embed)");
    println!("  Actual forward trace Q std:  {:.6} (using proper hidden state)", qc_std);

    if qc_std > 0.5 {
        println!("\n  FINDING: Manual trace produces healthy Q (std={:.4})", qc_std);
    ...
    println!("  Self-attn K cache: {} values, mean={:+.4}  std={:.4}",
             self_k_cached.len(), sk_mean, sk_std);
    println!("  Self-attn V cache: {} values, mean={:+.4}  std={:.4}",
             self_v_cached.len(), sv_mean, sv_std);
    ...
    println!("  Cross-attn K cache: {} values, mean={:+.4}  std={:.4}",
             cross_k_cached.len(), ck_mean, ck_std);
    println!("  Cross-attn V cache: {} values, mean={:+.4}  std={:.4}",
             cross_v_cached.len(), cv_mean, cv_std);
