...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        mel_mean, mel_std, mel_min, mel_max
    );
    ...
    println!(
        "  mean={:.6}  std={:.6}  min={:.6}  max={:.6}",
        enc_mean, enc_std, enc_min, enc_max
    );
    ...
    if enc_std < 0.01 {
        println!(
            "CONFIRMED: Encoder output is DEGENERATE (std={:.6} < 0.01)",
            enc_std
        );
        ...
        println!(
            "FALSIFIED: Encoder output is HEALTHY (std={:.6} > 0.1)",
            enc_std
        );
