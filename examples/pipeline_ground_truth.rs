...
fn delta_color(delta: f32) -> &'static str {
    if delta < 1.0 { "\x1b[32m" }      // Green
    else if delta < 5.0 { "\x1b[33m" } // Yellow
    else if delta < 20.0 { "\x1b[33m" } // Yellow
    else { "\x1b[31m" }                 // Red
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("│  A   │ Audio Input       │   {}    │ mean={:+.4} std={:.4} │ mean={:+.4} std={:.4} │",
             status_icon(delta_a),
             our_audio.mean, our_audio.std,
             gt.step_a_audio.mean, gt.step_a_audio.std);
    ...
    let mel_subset: Vec<f32> = mel_full.iter().take(n_frames_gt * n_mels).cloned().collect();
    ...
    println!("│  G   │ Encoder Output    │   ?    │ mean={:+.4} std={:.4} │ (need extraction)  │",
    ...
    let audio_region: Vec<f32> = (0..75).flat_map(|p| {
        encoded[p * d_model..(p + 1) * d_model].to_vec()
    }).collect();
    let padding_region: Vec<f32> = (1400..1500.min(enc_len)).flat_map(|p| {
        encoded[p * d_model..(p + 1) * d_model].to_vec()
    }).collect();
    ...
    println!("│  H   │ Enc Audio Region  │   {}    │ mean={:+.4} std={:.4} │ (should differ)    │",
             enc_status, audio_stats.mean, audio_stats.std);
    println!("│      │ Enc Padding Rgn   │        │ mean={:+.4} std={:.4} │ (from audio)       │",
    ...
    if delta_c > 5.0 {
        println!("│ {}✗ STEP C: Mel spectrogram diverges from ground truth by {:.1}%{}              │",
                 "\x1b[31m", delta_c, RESET);
        println!("│   → Check mel filterbank, log normalization, padding                        │");
    } else {
        println!("│ {}✓ STEP C: Mel spectrogram matches ground truth (delta={:.1}%){}               │",
                 "\x1b[32m", delta_c, RESET);
    }

    if enc_diff < 0.1 {
        println!("│ {}✗ STEP H: Encoder produces SAME output for audio and padding{}             │",
                 "\x1b[31m", RESET);
        ...
        println!("│ {}✓ STEP H: Encoder differentiates audio from padding (diff={:.2}){}            │",
