...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    use trueno_gpu::memory::resident::{kernel_cache_hits, kernel_cache_misses, reset_transfer_counters, TransferStats};
    ...
    println!("CPU model loaded in {:.1}ms", load_start.elapsed().as_millis());

    // Compute mel on CPU first
    let mel_start = Instant::now();
    let mel = model.compute_mel(&samples)?;
    println!("Mel computed in {:.1}ms ({} frames)", mel_start.elapsed().as_millis(), mel.len() / 80);

    // Convert to CUDA
    println!("\n=== Converting to CUDA ===");
    let cuda_start = Instant::now();
    let mut cuda_model = model.into_cuda(0)?;
    println!("CUDA model created in {:.1}ms", cuda_start.elapsed().as_millis());

    // Upload weights
    let upload_start = Instant::now();
    cuda_model.upload_encoder_weights()?;
    println!("Weights uploaded in {:.1}ms", upload_start.elapsed().as_millis());
    ...
    println!("Warmup: {:.1}ms (includes kernel JIT)", warmup_time.as_millis());

    // Get cache stats after warmup
    let hits_after_warmup = kernel_cache_hits();
    let misses_after_warmup = kernel_cache_misses();
    println!("Kernel cache: {} hits, {} misses (compiles)", hits_after_warmup, misses_after_warmup);
    ...
    for i in 0..n_runs {
        ...
        let run_layer_times: Vec<u128> = events.iter()
            .filter(|e| e.step.name() == "TRANSFORMER_BLOCK")
            .map(|e| e.duration_us as u128)
            .collect();

        println!("  Run {}: {:.1}ms ({} features, {} trace events)",
            i + 1,
            elapsed.as_millis(),
            features.len(),
            events.len()
    ...
    println!("  Average: {:.1}ms ({:.0}µs)", avg_time_us / 1000.0, avg_time_us);
    println!("  Best:    {:.1}ms ({:.0}µs)", min_time_us / 1000.0, min_time_us);
    ...
    let mut step_times: std::collections::HashMap<&str, Vec<u128>> = std::collections::HashMap::new();
    for event in last_events.iter() {
        step_times.entry(event.step.name())
            .or_default()
    ...
    for (step, times) in &step_times {
        ...
        println!("  {}: {} events, total {:.1}ms, avg {:.0}µs/event",
            step, times.len(), sum as f64 / 1000.0, avg);
    }

    println!("");
    println!("Total traced time: {:.1}ms", total_traced_us as f64 / 1000.0);
    ...
    println!("DARK MATTER:       {:.1}ms ({:.1}%)",
        unaccounted_us as f64 / 1000.0, unaccounted_pct);
    ...
    if let Some(last_layer_times) = layer_times.last() {
        ...
        let transformer_events: Vec<_> = last_events.iter()
            .filter(|e| e.step.name() == "TRANSFORMER_BLOCK")
            .collect();
    ...
    if unaccounted_pct.abs() > 20.0 {
        ...
        println!("   {:.1}% of time is unaccounted for in trace events.", unaccounted_pct.abs());
