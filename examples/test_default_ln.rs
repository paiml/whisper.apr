...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Set default layer norm weights
    let ln = model.decoder_mut().ln_post_mut();
    println!("Original decoder LN weight mean: {:.4}", ln.weight.iter().sum::<f32>() / ln.weight.len() as f32);
    
    // Reset to defaults: weight=1, bias=0
    for w in &mut ln.weight {
        *w = 1.0;
    }
    for b in &mut ln.bias {
        *b = 0.0;
    }
    
    println!("Reset to default: weight=1.0, bias=0.0");
    ...
    println!("\n=== Decoding with default LN weights ===\n");
